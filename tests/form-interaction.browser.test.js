...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`channel workbook uploads create persistent field tabs and channel pages (${viewport.width}px)`, { skip: !chromium }, async t => {
    ...
    const sourceRow = page.locator('#view tbody tr').filter({ has: page.locator('[data-t4sourceedit="测试新店"]') });
    assert.equal(await sourceRow.count(), 1);
    assert.equal(await sourceRow.locator('td').nth(0).innerText(), '测试新店');
    assert.equal(await sourceRow.locator('td').nth(2).innerText(), '测试新增渠道');
    assert.equal(await sourceRow.locator('td').nth(3).innerText(), '0');
    const summaryId = await sourceRow.locator('[data-t4sourceedit]').getAttribute('data-channel');
    await sourceRow.getByRole('button', { name: '归集损益', exact: true }).click();
    assert.equal(await page.locator('#t4DayCh').inputValue(), summaryId);
    ...
    const summaryRow = page.locator('#view tbody tr').filter({ has: page.locator(`[data-t4chedit="${summaryId}"]`) });
    assert.equal(await summaryRow.locator('td').nth(2).innerText(), '测试新增渠道');
    await summaryRow.getByRole('button', { name: '录入', exact: true }).click();
    assert.equal(await page.locator('#t4chSel').inputValue(), summaryId);
